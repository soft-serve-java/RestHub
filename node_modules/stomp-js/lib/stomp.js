...
function parse_frame(chunk) {
    ...
    while ( chunk.charAt(0) === '\r' || chunk.charAt(0) === '\n' ) chunk = chunk.substring(1);
...
function _connect(stomp) {
    ...
    if (stomp.ssl) {
        ...
        stomp.socket.connect({
            port : stomp.port, 
            host : stomp.host,
            localAddress : stomp.localAddress
        });
        _setupListeners(stomp);
    }
}
...
function _setupListeners(stomp) {
    function _connected() {
        ...
        if (utils.really_defined(stomp["heart-beat"])) {
            headers["heart-beat"] = stomp["heart-beat"];
        }
...
function Stomp(args) {
    ...
    this.localAddress = args['localAddress']
    this.debug = args['debug'];
    this.login = args['login'] || null;
    this.passcode = args['passcode'] || null;
    this['heart-beat'] = args['heart-beat'] || null;
...
Stomp.prototype.enableHeartBeat = function() {
    if ( ! this['heart-beat'] ) this['heart-beat'] = "60000,60000";
}
Stomp.prototype.heartBeat = function() {
    var self = this;
    if ( this.ecg ) clearTimeout(this.ecg);
    this.ecg = setTimeout(function() {
        self.socket.write("\n");
        self.heartBeat();
    }, this.ecgRate);
}
Stomp.prototype.skipHeartBeat = function() {
    if ( this.ecg ) {
        clearTimeout(this.ecg);
        this.heartBeat();
    }
}
...
Stomp.prototype.handle_new_frame = function(this_frame) {
    switch (this_frame.command) {
        ...
        case "CONNECTED":
            ...
            try {
                var heartBeatCliHdr = this['heart-beat'];
                var heartBeatSrvHdr = this_frame.headers['heart-beat'];
                if ( heartBeatSrvHdr && heartBeatCliHdr ) {
                    heartBeatsCli = heartBeatCliHdr.split(',');
                    heartBeatsSrv = heartBeatSrvHdr.split(',');
                    var cliRate = parseInt(heartBeatsCli[1]);
                    var srvRate = parseInt(heartBeatsSrv[1]);
                    if ( cliRate && srvRate ) {
                        // TODO not the offical algo yet
                        rate = cliRate < srvRate ? cliRate : srvRate;
                        if (this.debug) console.log("heart-beat rate " + rate);
                        this.ecgRate = rate;
                        this.heartBeat();
                    }
                }
            } catch(err) {
                if (this.debug) console.log("heart-beat error" + err);
            }
            this.emit('connected', this_frame);
            break;
        case "RECEIPT":
